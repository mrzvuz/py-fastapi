...
class BookRequest(BaseModel):
    id: Optional[int] = Field(
        default=None, description="ID is not needed when creating a book"
    )
...
async def update_book(book: BookRequest):
    ...
    if book.id is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Book ID is required."
        )

    updated_book = Book(**book.model_dump())

    for i, item in enumerate(BOOKS):
        if item.id == updated_book.id:
